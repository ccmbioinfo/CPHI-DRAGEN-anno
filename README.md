...
Note: Snakemake 9.16.2 stores its source cache in `~/.cache/snakemake` by default. If there is not enough space in your home directory, clear this cache between runs or redirect it to a location with sufficient space by adding the following line to `CPHI_DRAGEN_anno.sh` before the `snakemake` command:

```bash
export XDG_CACHE_HOME="path/to/.snakemake-cache"
```
